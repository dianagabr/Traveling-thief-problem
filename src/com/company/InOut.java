...
import com.company.Tsp.problem;
...
public class InOut {
    ...
    static File report, comp_report, stat_report, best_report_iteration, report_solution;
    private static Map<String, BufferedWriter> writer = new HashMap<String, BufferedWriter>();
    static CSVWriter report_iteration_writer, report_solution_writer;

    static String name_buf;
    static int opt;
    static boolean quiet_flag = false; /* --quiet was given in the command-line. */
         ...
         */
    {
        ...
        while (line != null) {
            ...
            if (!found_coord_section) {
                ...
                nodeptr[i].x = Integer.parseInt(city_info[1]);
                nodeptr[i].y = Integer.parseInt(city_info[2]);
                i++;
            }
   ...
   static void small_ttp_instance(){
        Ttp ttpInstance = new Ttp();
        Tsp tsp = new Tsp();
        Tsp.instance = new Tsp.problem();
        Tsp.point[] nodeptr = null;
   }
    static void read_TTP_instances(String ttp_filename) throws IOException {
        ...
        int[] items_profits = null;
        int[] items_weights = null;




        if (ttp_filename == null) {
            System.err.println("No instance file specified, abort");
            System.exit(1);
        }

        if (!new File(ttp_filename).canRead()) {
            System.err.println("Can not read file " + ttp_filename);
            System.exit(1);
        }

        System.out.println("\nreading ttp-file " + ttp_filename + " ... ");


        int i = 0;
        boolean found_coord_section = false;
        boolean found_items_section = false;
        Reader reader = new InputStreamReader(new FileInputStream(ttp_filename), "UTF8");
        BufferedReader bufferedReader = new BufferedReader(reader);
        String line = bufferedReader.readLine();

        while(! found_coord_section){
            if (line.startsWith("PROBLEM NAME")) {
                    Ttp.problemName = line.split(":")[1].trim();
                } else if (line.startsWith("KNAPSACK DATA TYPE")) {
                    Ttp.knapsackDataType=line.split(":")[1].trim();
                } else if (line.startsWith("DIMENSION")) {
                    Tsp.n = Integer.parseInt(line.split(":")[1].trim());
                    Tsp.instance.n = Tsp.n;
                    ...
                    assert (Tsp.n > 2 && Tsp.n < 6000);
                     ...
                     Kp.m = Integer.parseInt(line.split(":")[1].trim());
                     items_profits = new int[  Kp.m ];
                     items_weights = new int[  Kp.m ];
                     Kp.pickingPlan = new int[ Kp.m];
                     Kp.availability = new int[Kp.m];

                }else if(line.startsWith("CAPACITY OF KNAPSACK")){
                   Kp.W = Integer.parseInt(line.split(":")[1].trim());
                }else if(line.startsWith("MIN SPEED")){
                   Ttp.MINIMUM_SPEED =  Double.parseDouble(line.split(":")[1].trim());
                }else if(line.startsWith("MAX SPEED")){
                Ttp.MAXIMUM_SPEED =Double.parseDouble(line.split(":")[1].trim());
                }else if(line.startsWith("RENTING RATIO")){
                    Ttp.RATE = Double.parseDouble(line.split(":")[1].trim());
                }
                else if (line.startsWith("EDGE_WEIGHT_TYPE")) {
                    buf = line.split(":")[1].trim();
                    if (buf.equals("EUC_2D")) {
        ...
        for(int item = 0; item < Kp.m ; item++) {
          Kp.availability[item] = -1 ;
        }
        ...
        while (line != null) {
            ...
            items_profits[i] = Integer.parseInt(item_info[1]);
            items_weights[i] = Integer.parseInt(item_info[2]);
            int city_index = Integer.parseInt(item_info[3]) - 1;
            Kp.availability[i] = city_index;
            i++;
        ...
        Kp.profits = items_profits;
        Kp.weights = items_weights;

        ttpInstance.compute_speed();
        Tsp.instance.nodeptr = nodeptr;
        Ttp.BOOST = true;
    ...
    static void write_report()
         ...
         */
    {
        System.out.println("best " + Ants.best_so_far_ant.score + ", iteration: " + iteration + ", time "
                + Timer.elapsed_time());
        if (comp_report != null)
            printToFile(comp_report, "best " + Ants.best_so_far_ant.score + "\t iteration " + iteration
    ...
    static void population_statistics()
        /*
         * FUNCTION: compute some population statistics like average tour length, average score,
         * standard deviations for tour_length and score, average distance, branching-factor and
         ...
         */
    {
        ...
        double[] l;
        int[] h;
        double pop_mean_l, pop_stddev_l, pop_mean_s, pop_stddev_s, avg_distance = 0.0;

        l = new double[Ants.n_ants];
        for (k = 0; k < Ants.n_ants; k++) {
            l[k] = Ants.ant[k].score;
        }


        h = new int[Ants.n_ants];
        for (k = 0; k < Ants.n_ants; k++) {
            h[k] = Ants.ant[k].tour_length;
        }

        // mean for tour length
        pop_mean_l =Utilities.mean(h, Ants.n_ants);
        pop_stddev_l = Utilities.std_deviation(h, Ants.n_ants, pop_mean_l);


        //mean for score
        pop_mean_s = Utilities.meanr(l, Ants.n_ants);
        pop_stddev_s = Utilities.std_deviationr(l, Ants.n_ants, pop_mean_s);
        ...
        if (stat_report != null) {
            printToFile(stat_report, iteration + "\t" + pop_mean_l + "\t" + pop_stddev_l + "\t" + (pop_stddev_l / pop_mean_l)
         ...
         */
    {
        ...
        System.out.println("Best Solution in try " + ntry + " is " + Ants.best_so_far_ant.score);

        if (report != null)
            printToFile(report, "Best: " + Ants.best_so_far_ant.score + "\t Iterations: " + found_best
        ...
        if(report_solution != null){
            //write in csv
            String[] values = {Double.toString(Ants.best_so_far_ant.score), Integer.toString(Ants.best_so_far_ant.tour_length), Integer.toString(found_best), Double.toString(time_used), Integer.toString(ntry) };
            report_iteration_writer.writeNext(values);
        }


        best_in_try[ntry] = Ants.best_so_far_ant.tour_length;
        best_score_try[ntry] = Ants.best_so_far_ant.score;
        ...
        aw_best_picking_plan_in_try[ntry] = Arrays.toString(Ants.best_so_far_ant.picking_plan);




        System.out.println("\ntry " + ntry + ", Best " + best_score_try[ntry] + ", found at iteration "
         ...
         */
    {
        ...
        best_score = Utilities.best_of_scores(best_score_try, max_tries);
        worst_score = Utilities.worst_of_scores(best_score_try, max_tries);
        ...
        avg_sol_quality = Utilities.meanr(best_score_try, max_tries);
        stddev_best = Utilities.std_deviationr(best_score_try, max_tries, avg_sol_quality);
        ...
        if (report != null) {
            ...
            printToFile(report, "Best tour length: " + best_tour_length + "\t\t Worst tour length: " + worst_tour_length);
            printToFile(report, "Best score: " + best_score + "\t\t Worst score: " + worst_score);
        ...
        if (comp_report != null)
            printToFile(comp_report, "end problem " + Ttp.problemName);
        ...
        try {
            report_iteration_writer.close();
            report_solution_writer.close();
        } catch (IOException e) {
            System.err.println("Could not close file " + report_iteration_writer + " " + e.getMessage());
        }
    }
         ...
         */
    {
      ...
      //  Parse.parse_commandline(args);
        ...
        best_score_try = new double[max_tries];
        ...
        aw_best_picking_plan_in_try = new String[max_tries];
        ...
        if (quiet_flag == false) {
            Writer w;
            try {
                temp_buffer = "best." + Ttp.problemName;
                ...
                //adding header
                String[] header = { "best score", "best length",  "iteration", "time", "try" };
                report_iteration_writer.writeNext(header);
