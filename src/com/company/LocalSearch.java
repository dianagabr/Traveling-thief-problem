...
public class LocalSearch {
     ...
     */
    {
        ...
        int item ;
        for(item = 0; item < Kp.m; item ++){
              if(bestPickingPlan[item] == 1){
                  pickingPlan[item] = 0;
              }else{
                  pickingPlan[item] = 1;
              ...
              if(objValue > bestObjValue && total_weight <= Kp.W){
                  ...
                  pickingPlan[item] = bestPickingPlan[item];
              }
        }
      ...
      */
    {
        ...
        int[] tour_2 = new int[tour.length];
        for( a = Tsp.n -1 ;  a >= 0; a --){
            double objValue = Ttp.calculateObjectiveValue(tour,pickingPlan);
            for(b = a -1 ; b >= 0 ; b-- ){
                tour_1 = tour.clone();
                Utilities.insert_city(tour_1, a, b);
                if( b == 0){
                    tour_1[Tsp.n] = tour_1[0];
                }
                double newObjValue = Ttp.calculateObjectiveValue(tour_1, pickingPlan);
                if( newObjValue  > objValue ){
                    objValue = newObjValue;
                    tour_2 = tour_1.clone();
                }
                if( b == 0 && Kp.check_if_empty(tour_2) == false){
                    tour = tour_2.clone();
                }
      ...
      */
    {
        ...
        int[] newPickingPlan  = new int[Kp.m];
        double newObjValue = Ttp.calculateObjectiveValue(tour, newPickingPlan);
        int[] pickingPlan;

        int counter;

        for(counter = 0 ; counter < numberOfIterations; counter ++) {
            pickingPlan = new int[Kp.m];
            for (int i = 0; i < Kp.m; i++) {
                double random = Utilities.ran01(Utilities.seed);
                if (random < probability) {
                    if (newPickingPlan[i] == 0) {
                        pickingPlan[i] = 1;
                    } else {
                        pickingPlan[i] = 0;
                    }
            ...
            double objValue = Ttp.calculateObjectiveValue(tour, pickingPlan);
            if(Kp.check_if_empty(pickingPlan) == false){
                if(Kp.check_if_empty(newPickingPlan) == true){
