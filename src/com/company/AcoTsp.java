...
public class AcoTsp {
    ...
    static void construct_picking_plan()
        /*
         * FUNCTION: manage the picking plan construction phase
         * INPUT: none
         * OUTPUT: none
         * (SIDE)EFFECTS: when finished, all ants of the colony have constructed a picking plan
         */
    {
        int k ;
        for( k = 0 ; k < Ants.n_ants; k++){
            if(!Ttp.tourObjValue.containsKey(Ants.ant[k].tour)){
                ...
                Ants.ant[k].score = Ttp.tourObjValue.get(Ants.ant[k].tour);
            }
        }
    }
         ...
         */
    {
        ...
        // TRACE ( System.out.println("Utilities.INITIALIZE Utilities.TRIAL\n"); );
        ...
        InOut.lambda = 0.05;
        Ants.best_so_far_ant.score = Integer.MIN_VALUE;
        InOut.found_best = 0;
    ...
    static void update_statistics_ttp()
        /*
         * FUNCTION: manage some statistical information about the trial, especially
         * if a new best solution (best-so-far or restart-best) is found and
         * adjust some parameters if a new best solution is found
         ...
         */
    {
        ...
        iteration_best_ant = Ants.find_best_ttp(); /* iteration_best_ant is a global variable */

        if (Ants.ant[iteration_best_ant].score > Ants.best_so_far_ant.score) {
            ...
            if (Ants.mmas_flag) {
                if (LocalSearch.ls_flag == 0) {
                    p_x = Math.exp(Math.log(0.05) / Tsp.n);
                    Ants.trail_min = 1. * (1. - p_x) / (p_x * (double) ((Ants.nn_ants + 1) / 2));
                    Ants.trail_max = 1. / ((Ants.rho) * Ants.best_so_far_ant.score);
                    Ants.trail_0 = Ants.trail_max;
                    Ants.trail_min = Ants.trail_max * Ants.trail_min;
                } else {
                    Ants.trail_max = 1. / ((Ants.rho) * Ants.best_so_far_ant.score);
        ...
        if (Ants.ant[iteration_best_ant].score > Ants.restart_best_ant.score) {
            Ants.copy_from_to(Ants.ant[iteration_best_ant], Ants.restart_best_ant);
            InOut.restart_found_best = InOut.iteration;
            System.out.println("restart best: " + Ants.restart_best_ant.score + " restart_found_best "
                    + InOut.restart_found_best + ", time " + Timer.elapsed_time());
        }
    }
         ...
         */
    {
        ...
        if ((InOut.iteration % 100) == 0) {
            ...
            System.out.println("best so far " + Ants.best_so_far_ant.score + ", iteration: " + InOut.iteration
            ...
            if (Ants.mmas_flag && (InOut.branching_factor < InOut.branch_fac)
                ...
                Ants.restart_best_ant.score = Integer.MIN_VALUE;
                Ants.init_pheromone_trails(Ants.trail_max);
                Ants.compute_total_information();
         ...
         */
    {
        ...
        if (InOut.iteration % Ants.u_gb == 0) {
            iteration_best_ant = Ants.find_best_ttp();
            Ants.global_update_pheromone_ttp(Ants.ant[iteration_best_ant]);
        } else {
            if (Ants.u_gb == 1 && (InOut.iteration - InOut.restart_found_best > 50))
                Ants.global_update_pheromone_ttp(Ants.best_so_far_ant);
            else
                Ants.global_update_pheromone_ttp(Ants.restart_best_ant);
        }
    ...
    public static void main(String[] args) {
        ...
        double aw_best_score = Utilities.best_of_scores(InOut.best_score_try, InOut.max_tries);
        String aw_best_tour = InOut.aw_best_tour_in_try[Utilities.aw_best_tour_index()];
        String aw_best_picking_plan = InOut.aw_best_picking_plan_in_try[Utilities.aw_best_picking_plan_index()];
        try {
            Writer w = new OutputStreamWriter(new FileOutputStream("solution." + Ttp.problemName), "UTF8");
            BufferedWriter out = new BufferedWriter(w);
            out.write(aw_best_tour_length + "\n");
            out.write(aw_best_tour + "\n");
            out.write(aw_best_score  + "\n");
            out.write(aw_best_picking_plan + "\n");
            out.close();
        } catch (IOException e) {
            System.err.print("Could not write file solution." + Ttp.problemName + " " + e.getMessage());
            System.exit(1);
        }
        System.out.println();
        ...
        System.out.println("Best picking_plan");
        System.out.println(aw_best_picking_plan);
        System.out.println(aw_best_score);
