...
public class Utilities {
    ...
    static double worst_of_scores(double[] values, int l){
        double min;
        int k;
    ...
    static double best_of_scores(double[] values, int l)
        /*
         * FUNCTION: return the maximum value in an double value
         * INPUT: pointer to array, length of array
    ...
    static void insert_city(int[] tour, int position, int new_position){

        int city = tour[position];
        for(int i = position -1 ; i >= new_position ; i --) {
            tour[i+1] = tour[i];
        }
        tour[new_position] = city;
    ...
    static int aw_best_picking_plan_index() {
        double max;
        int k;

        double[] values = InOut.best_score_try;
        int l = InOut.max_tries;
