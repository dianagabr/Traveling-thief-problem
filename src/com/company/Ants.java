...
public class Ants {
    ...
    static class ant_struct {
        int[] tour;
        int[] picking_plan; //the corresponding picking plan for tour
        double score; //the corresponding score for tour
        boolean[] visited;
        int tour_length;
    }
    ...
    static void global_update_pheromone_ttp(ant_struct a)
        /*
         * FUNCTION: reinforces edges used in ant k's solution
         * INPUT: pointer to ant that updates the pheromone trail
         * OUTPUT: none
    ...
    static void copy_from_to(ant_struct a1, ant_struct a2) {
        ...
        a2.score = a1.score;
        for( i = 0 ; i < Kp.m; i++){
            a2.picking_plan[i] = a1.picking_plan[i];
        }
        for (i = 0; i < Tsp.n; i++) {
            a2.tour[i] = a1.tour[i];
        }
